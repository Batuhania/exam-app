...
const CheatSheet: React.FC = () => {
    ...
    const filteredSheets = cheatSheets.map(section => {
        ...
        // Return section only if title matches or has matching items
        if (section.title.toLowerCase().includes(searchTerm.toLowerCase()) || matchingItems.length > 0) {
            return {
                ...
                // Note: If searched by section title, show all items. If searched by content, show matching items.
                // Improvement: If section title matches, keep all. If not, keep only matching items.
            };
        }
        return null;
    ...
    return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8 pb-24">
            ...
            <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredSheets.map((section: any) => (
